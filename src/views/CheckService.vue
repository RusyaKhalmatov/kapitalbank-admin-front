    ...
    <div class="main">
        <h1 class="headline">Информация об оплате сервиса</h1>
            ...
            <div class="content" v-if="!isEmpty">
                ...
                >
                    <template slot="items" slot-scope="props">
                        <tr>
                            ...
                            <td>{{props.item.senderAmount / Math.pow(10, props.item.senderCurrency.scale) | number-format}} {{props.item.senderCurrency.name}}</td>
                            <td>{{props.item.senderFeeAmount / 100 | number-format}} UZS</td>
                            <td>{{props.item.receiver }}</td>
                            <td>{{props.item.receiverFullName}}</td>
                            <td>{{props.item.receiverAmount / Math.pow(10, props.item.receiverCurrency.scale) | number-format}} {{props.item.receiverCurrency.name}}</td>                        
                ...
                <h3 class="mt-3">Состояние в Paynet: №: {{responseIdentity.identity}} - {{responseIdentity.status}} - {{responseIdentity.statusMessage}}</h3>
...
export default {
    ...
    methods:{
        checkService(){
            ...
            this.isEmpty = true;
            this.$http.post(this.$store.getters.apiUrl+`/transaction/manual/check-service/${this.operationId}`)
            .then(response=>{
                this.loader = false;
                this.isEmpty = false;
