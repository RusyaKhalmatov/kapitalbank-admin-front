                    ...
                    <v-flex xs12>
                        ...
                        <div class="search-box">
                            <span class="input-text">
                                <v-checkbox class="search-box-child" v-model="isServiceId" label="По ID сервиса"></v-checkbox>
                                <v-text-field
                                v-if="isServiceId"
                            ...
                            <span class="input-text">
                                <v-checkbox class="search-box-child" v-model="isNameRu" label="По наименованию"></v-checkbox>
                                <v-text-field
                                v-if="isNameRu"
                                class="search-box-child"
                                v-model="nameRu" 
                            ...
                            <span class="input-text">
                                <v-checkbox class="search-box-child" v-model="isActive" label="По активности"></v-checkbox>   
                                <v-radio-group style="margin:0; padding: 0" class="radio" v-model="active" v-if="isActive">
                                    <v-radio
                                        :key="0"
                                        label="Включен"    
                                        :value="true"                
    ...
    export default {
        ...
        methods: {
            ...
            getServices(page) {
                ...
                if(!this.isActive)
                    this.active = '';
                this.$http.get(this.$store.getters.newApiUrl+`/payment-service?page=${page-1}&size=10&serviceId=${this.serviceId.trim()}&nameRu=${this.nameRu.trim()}&active=${this.active}`)
                .then(response=>{
                    this.loader = false;
            ...
            editService(val){
                this.$http.get(this.$store.getters.newApiUrl+`/payment-service/${val.serviceId}`)
                .then(response=>{
            ...
            updateServices() {
                let self = this;
                self.$http.get(self.$store.getters.apiUrl + '/service/update')
                    .then(() => {
                        location.reload();
                    }, self.handleError);
            }
