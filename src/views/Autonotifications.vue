  ...
  <div class="main">
    ...
    <v-data-table :headers="headers" :items="content" :pagination.sync="pagination">
      <template slot="items" slot-scope="props">
        <tr>
          <td>
            <v-btn depressed small :loading="loader" @click="getUsers(props.item.id)">Получить</v-btn>
          </td>
          <td>{{ props.item.id}}</td>
    ...
    <v-dialog v-model="dialog" persistent max-width="650px">
      <v-card>
        ...
        <v-card-text>
          ...
          <v-select v-model="data.operationCodeId" label="Тип операции" multiple :items="typesData"
                    item-text="operationType" item-value="id"></v-select>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      getUsers(id) {
        this.loader = true;
        this.$http.get(`${this.prodApiUrl}/autonotifications/users/${id}`)
          .then(response => {
      ...
      remove() {
        this.loader = true;
        this.$http.post(`${this.prodApiUrl}/autonotifications/delete/${this.id}`)
          .then(response => {
            // console.log(response.data.data)
      ...
      edit() {
        ...
        this.$http.post(`${this.prodApiUrl}/autonotifications/save`, this.data)
          .then(response => {
            // console.log(response.data.data)
            this.successMessage("Успешно")
            this.dialog = false;
      ...
      getList() {
        this.$http.get(`${this.prodApiUrl}/autonotifications`)
          .then(response => {
            // console.log(response.data.data)
      ...
      getTypes() {
        this.$http.get(`${this.prodApiUrl}/autonotifications/operationTypes`)
          .then(response => {
            // console.log(response.data.data)
