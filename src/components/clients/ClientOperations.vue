        ...
        <div class="content" @scroll="scrollFunc">     
            <div v-for="(item, index) in monitoringData" :key="index">
                <span :class="['operation', { 'bg-panel': index % 2 != 0, 'active-class': active===index}]" >
                    ...
                    <p @click="getTransactions(item.operationId, index)" class="operation-child">{{item.amount  | number-format}} {{item.currency}}</p>
                    <p @click="getTransactions(item.operationId, index)" class="operation-child">{{item.operationType}}</p>
                    
                    <p @click="getTransactions(item.operationId, index)" class="operation-child">{{item.senderPan}}</p>
                    <p @click="getTransactions(item.operationId, index)" class="operation-child">{{item.senderName}}</p>
                    <p @click="getTransactions(item.operationId, index)" class="operation-child">{{item.receiverPan}}</p>
                    <p @click="getTransactions(item.operationId, index)" class="operation-child">{{item.receiverName}}</p>
                    <p @click="getTransactions(item.operationId, index)" class="operation-child">{{item.feeAmount | number-format}} {{item.feeCurrency}}</p>
                    ...
                    <span @click="getTransactions(item.operationId, index)" class="operation-child" style="display: flex; flex-direction: column; align-items: center;">
                        <p  style="font-size:10px; padding: 8px; width: 130px; text-overflow: ellipsis; overflow: hidden;" class="mb-0">{{item.cardToken}}</p>
                        <v-btn v-if="item.cardToken!=' '" small depressed v-clipboard:copy="item.cardToken" v-clipboard:success="textOnCopy" v-clipboard:error="textOnError">Скопировать </v-btn>
                    </span>
                    <span class="operation-child" style="display: flex; flex-direction: column; align-items: center;">
                        ...
                        <v-btn small depressed v-clipboard:copy="item.operationId" v-clipboard:success="textOnCopy" v-clipboard:error="textOnError">Скопировать </v-btn>
                    </span>
