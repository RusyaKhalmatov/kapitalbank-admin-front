<template>
    <v-layout column wrap>
        <v-flex xs12>
            <div class="main">
                <v-card-text>
                        <h1 class="headline">Beeline отчет</h1>
                    ...
                    <v-layout row wrap>
                        ...
                        <div class="amount-box" v-if="operationAmount.amount">
                            <span class="amount-box-child">
                                <span class="d-flex align-center column">
                                    <p class="amount-text">Общая сумма:</p>
                                    <p class="amount-value">{{operationAmount.amount/100|number-format}}</p>
                                </span>
                                <span class="d-flex align-center column">
                                    <p class="amount-text">Комиссия:</p>
                                    <p class="amount-value">{{operationAmount.fee/100|number-format}}</p>
                                </span>
                            </span>  
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            loadAmount() {
                let self = this;                
                self.$http.post(self.$store.getters.beelineUrl + '/apelsin/transaction/amount', self.data, {headers:{Authorization: 'Basic YmVlbGluZS1rYXBpdGFsOmNXQTM4JUpALFllR2U9VDY='}})
                    .then(response => {
            ...
            getExcel() {
                ...
                if(this.date.toDate==="")
                    delete this.data.dateTo
                self.$http.post(self.$store.getters.beelineUrl + `/apelsin/transaction/excel`, self.data,{headers:{
                    Authorization: 'Basic YmVlbGluZS1rYXBpdGFsOmNXQTM4JUpALFllR2U9VDY=',
                }})
                    .then(response => {
                        self.loader = false;
            ...
            loadReports(page) {
                ...
                self.$http.post(self.$store.getters.beelineUrl + `/apelsin/transaction?page=${page-1}&size=10`, self.data, {headers:{
                    Authorization: 'Basic YmVlbGluZS1rYXBpdGFsOmNXQTM4JUpALFllR2U9VDY=',
                }})
                    .then(response => {
                        self.loader = false;
