<template>
    <v-layout column wrap>
        <v-flex xs12>
            <div class="main">
                <v-card-text>
                        <!-- <h1 class="headline">Ucell отчет</h1> -->
                    ...
                    <v-layout row wrap>
                        ...
                        <div class="amount-box" v-if="operationAmount.totalAmount">
                            <span class="amount-box-child">
                                ...
                                <span class="d-flex align-center column">
                                    <p class="amount-text">Общая комиссия:</p>
                                    <p class="amount-value">{{operationAmount.grossFee/100|number-format}}</p>
                                </span>
                                <span class="d-flex align-center column">
                                    <p class="amount-text">Чистая комиссия (Uzcard):</p>
                                    <p class="amount-value">{{operationAmount.netFeeUzCard/100|number-format}}</p>
                                </span>
                                <span class="d-flex align-center column">
                                    <p class="amount-text">Чистая комиссия (Humo):</p>
                                    <p class="amount-value">{{operationAmount.netFeeHumo/100|number-format}}</p>
                                </span>
                            </span>                                                     
                        </div>                  
                        <template v-if="false">                        
                            ...
                            <v-data-table
                                ...
                                <template slot="items" slot-scope="props">
                                    <tr @click="props.expanded = !props.expanded">
                                        ...
                                        <td>{{ props.item.grossCommission/100| number-format}}</td>
                                        <td>{{ props.item.netCommission/100|number-format}}</td>
    ...
    export default {
        ...
        data() {
            return {
                ...
                operationsHeaders: [
                    ...
                    {text: "Имя сервиса", value: "serviceName"},
                    {text: "Имя шлюза", value: "gatewayName"},
                    {text: "Получатель", value: "details"},                   
                    {text: "Сумма", value: "amount"},
                    {text: 'Общая комиссия', value: "grossCommission"},
                    {text: "Чистая комиссия", value: "netCommission"},
                    {text: "Статус", value: "status"},
                    {text: "Дата и время", value: "time"},
                ],
                filtersTypeTemp: [],
                statusData: [],
                operationType: [],
        ...
        methods: {
            ...
            loadAmount() {
                ...
                self.$http.get("https://ek.kapitalbank.uz/api/reports/getAll/amountAndFee/", {headers:{Authorization: 'Basic dWNlbGwta2FwaXRhbDo1WD8nJWRWezdEcyNAdnBW'},
                params: data})
                    .then(response => {
            ...
            getExcel() {
                ...
                self.$http.get(self.$store.getters.ucellUrl + `/reports/getAll/excel`,{headers:{
            ...
            loadReports(page) {
                ...
                self.$http.get(self.$store.getters.ucellUrl + `/reports/getAll?page=${page-1}&size=10`, {headers:{
                    Authorization: 'Basic dWNlbGwta2FwaXRhbDo1WD8nJWRWezdEcyNAdnBW',
                }, params: data})
                    .then(response => {
                        self.loader = false;
