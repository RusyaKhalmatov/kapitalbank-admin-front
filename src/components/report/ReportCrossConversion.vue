    ...
    <v-flex xs12>
        <div class="main">
            <v-card-title>
                ...
                <h1 class="headline">Отчет по кросс-конвертации валют</h1>
                ...
                <v-layout row wrap>
                    <v-flex xs12>
                        ...
                        <v-flex xs12>
                            ...
                            <v-card class="d-flex flex-wrap" style="margin-top: 20px">
                                <v-card-title style="font-size: large;">Целевая валюта:</v-card-title>
                                <v-checkbox
                                    v-for="(currency, index) in refCurrencies"
                                    :key="index"
    ...
    data() {
        return {
            ...
            postData : {
                ...
                targetCurrencies: this.refCurrencies,
            },
            totalPages: 0,
            page: 1,
            loader: false,
    ...
    watch: {
        ...
        targetCurrencies(currencies) {
            ...
            if (currencies.length === 0) {
                this.targetCurrencies = this.refCurrencies;
            }
        }
    },
    methods: {
        ...
        preparePostData() {
            this.postData = {
                ...
                operationType: this.operationType,
                statuses: this.statuses,
                targetCurrencies: this.targetCurrencies,
            };
        },
        getStatuses(){
        ...
        getExcel(){
            ...
            this.excelData = [];
            this.$http.post(
                this.$store.getters.newApiUrl2 + `/report/crossconversion/excel`,
        ...
        getOperationTypes() {
            this.operationType = [];
            this.$http.get(this.$store.getters.newApiUrl2 + '/report/crossconversion/operationTypes')
                .then(response => {
        ...
        getCurrencies() {
            ...
            this.$http.get(this.$store.getters.newApiUrl2 + '/report/crossconversion/currencies')
                .then(response => {
                    this.refCurrencies = response.data.data;
                    this.targetCurrencies = response.data.data;
                }, this.handleError);
        }
    },
    computed: {
        reportFileName() {
            return "crossconversion_" + (new Date()).toLocaleDateString() + ".xls"
        },
        conversionExport() {
            let data = {};
