    ...
    <v-dialog v-model="editDialog" persistent max-width="560">
      <v-card>
        ...
        <v-card-text style="height: 300px; overflow: auto">
          ...
          <v-select :items="operationCodeData" v-model="operationCode" item-text="operationCode"
                    item-value="operationCode"></v-select>
...
<script>
  ...
  export default {
    ...
    methods: {
      remove() {
        let self = this;
        self.$http.delete(self.prodApiUrl + '/limit/partialVip/' + this.id)
          .then(() => {
      ...
      getOperationCode() {
        let self = this;
        self.$http.get(self.prodApiUrl + '/operationCode')
          .then((response) => {
      ...
      getPartialVipClients() {
        let self = this;
        self.$http.get(self.prodApiUrl + '/limit/partialVip')
          .then((response) => {
            console.log(response.data.data);
