    ...
    <v-data-table
      ...
      <template slot="items" slot-scope="props">
        <tr>
          ...
          <td>{{
              props.item.login.split(':').length === 2 ? props.item.login.split(':')[0] + ' (в.у.:' + new Date(parseInt(props.item.login.split(':')[1])).toLocaleString() + ')' : props.item.login
            }}
          </td>
          ...
          <td style="min-width:292px">
            ...
            <v-btn @click="openEditBox(props.item)" small icon>
              <v-icon small>mdi-pencil</v-icon>
            </v-btn>
            <v-btn @click="openRemoveBox(props.item)" small icon>
    ...
    <v-dialog v-model="dialog" persistent max-width="650px">
      <v-card>
        ...
        <v-card-text>
          ...
          <v-text-field v-model="data.phone" label="Номер телефона" placeholder="9989########"
                        maxlength="12"></v-text-field>
          <v-text-field v-model="data.email" label="Email"></v-text-field>
          <v-select v-model="data.branchId" :items="branches" item-text="name" item-value="branchId"
...
export default {
  ...
  methods: {
    getRoles() {
      this.$http.get(this.$store.getters.newApiUrl2 + `/card-employee/v2/user-role`)
        .then(response => {
    ...
    edit() {
      this.loader = true;
      this.$http.put(this.$store.getters.newApiUrl2 + `/card-employee/v2/${this.id}`, this.data)
        .then(response => {
    ...
    add() {
      this.loader = true;
      this.$http.post(this.$store.getters.newApiUrl2 + `/card-employee/v2`, this.data)
        .then(response => {
          this.getList(this.page)
          this.dialog = false;
          this.loader = false;
    ...
    remove() {
      this.loader = true;
      this.$http.delete(this.$store.getters.newApiUrl2 + `/card-employee/v2/${this.id}`)
        .then(response => {
          this.getList(this.page);
    ...
    getList(page) {
      this.loader = true;
      this.$http.post(this.$store.getters.newApiUrl2 + `/card-employee/v2/pageable?page=${page - 1}&size=10&search=${this.search}`)
        .then(response => {
    ...
    getExcel() {
      this.loader = true;
      this.$http.post(this.$store.getters.newApiUrl2 + `/card-employee/v2/excel?search=${this.search}`)
        .then(response => {
    ...
    resetPassword() {
      this.loader = true;
      this.$http.post(this.$store.getters.newApiUrl2 + `/card-employee/v2/${this.id}/reset-password`)
        .then(response => {
          console.log(response.data.data)
