<template>
    <div>
        <div class="head">
            <h2 class="mt-3">Виртульная карта -  КРЕДИТ</h2> 
            <v-btn small class="excel-btn" @click="getExcel">Скачать Excel</v-btn>           
            <download-excel
                    v-show="false"
                    id="excel-credit"
                    name="virtualcards_credit"
...
export default {
    ...
    watch:{
        page(val){
            this.virtualCardsCredit(val);
        },
        date(val){
            if(this.page===1){
                this.virtualCardsCredit(1);
                this.getAmount();
            }
            else{
                this.page = 1;  
    ...
    methods:{
        getExcel(){
            ...
            if(this.cardProduct.length!=0){
                this.$http.post(this.$store.getters.newApiUrl2+`/report/virtual_card/credit/excel`, postData)
                .then(response=>{
                    this.excelData = response.data.data;  
                    if(this.excelData.length!=0){
                        let elem = document.getElementById('excel-credit');
                        setTimeout(()=>{
                            elem.click();
                        }, 500)
                    }
        ...
        getAmount(){
            ...
            if(this.cardProduct.length!=0){
                this.$http.post(this.$store.getters.newApiUrl2+`/report/virtual_card/credit/amount`, postData)
                .then(response=>{
        ...
        virtualCardsCredit(page){
            ...
            if(this.cardProduct.length!=0){
                ...
                this.$http.post(this.$store.getters.newApiUrl2+`/report/virtual_card/credit?page=${page-1}&size=10`, postData)
                .then(response=>{
                    // console.log(response.data.data);
