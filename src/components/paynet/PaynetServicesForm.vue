        ...
        <v-layout row wrap>
          ...
          <v-tabs style="width:100%" v-model="tabs">
            ...
            <v-tab-item class="pa-5">
              <div>
                <v-flex xs12>
                  <v-select v-model="localPaymentCategoriesIds" multiple :items="localPaymentsCategories"
                            item-text="nameRu" item-value="id" label="Категории для оплаты на местах"></v-select>
                </v-flex>
                <v-flex xs12>
                  <v-text-field
                ...
                <v-flex xs12 v-if="serviceFormItems.id">
                  ...
                  <work-schedule :is="workSchedule" :serviceId="serviceFormItems.serviceId"
                                 :id="serviceFormItems.id"></work-schedule>
                </v-flex>
                <v-flex xs12>
                  <v-checkbox
          ...
          <div class="fields-box">
            ...
            <div v-if="isShow" class="paynetServiceFieldsDto">
                                <span v-if="isAdd&&serviceFormItems.paynetServiceFields.length!=0"
                                      class="d-flex fields-header">
              ...
              <span class="d-flex fields-data" v-for="(item,index) in serviceFormItems.paynetServiceFields"
                    :key="index">
                                    ...
                                    <p>
                                        <v-btn @click="editField(item)" icon small><v-icon
                                          small>mdi-pencil</v-icon></v-btn>
                                        <v-btn @click="removeField(item)" icon small><v-icon
            ...
            <span v-if="isAdd" class="input-box">
                                ...
                                <span class="fields-input">
                                    ...
                                    <v-select v-model="fieldType" label="fieldType" :items="['S', 'N','E']"></v-select>                                
                                </span>
                                <span class="fields-input">
          ...
          <v-layout column wrap>
            <v-flex xs12>
              <paynet-auto-invoice :invoiceData.sync="invoiceData"
                                   :serviceFormItems="serviceFormItems"></paynet-auto-invoice>
            </v-flex>
            <v-flex xs12>
              <v-checkbox
            ...
            <v-flex xs12>
              ...
              <v-select style="width:300px" v-model="serviceFormItems.serviceBilling" label="Биллинг"
                        :items="billingTypesData"></v-select>
            </v-flex>
          </v-layout>
          <v-flex xs12>
...
import paynetAutoInvoice from './paynetServices/paynetAutoInvoice'

export default {
  name: "PaynetServicesForm",
  components: {
    workSchedule, paynetAutoInvoice
  },
  data() {
    return {
      serviceFormItems: {
  ...
  methods: {
    ...
    removeField(field) {
      this.isShow = false;
      console.log('test')
      const item = this.serviceFormItems.paynetServiceFields.find(x => x.id === field.id);
      if (item) {
        console.log('test1')

        this.$http.delete(this.$store.getters.newApiUrl + `/payment-service/${this.serviceFormItems.serviceId}/field/${field.id}`)
          .then((response) => {
        ...
        if (this.fieldId) {
          ...
          this.$http.put(this.$store.getters.newApiUrl + `/payment-service/${this.serviceFormItems.serviceId}/field/${this.fieldId}`, putData)
            .then((response) => {
              ...
              // .then(response=>{
              let newFields = response.data.data
              const item = this.serviceFormItems.paynetServiceFields.find(x => x.id === this.fieldId);
              this.serviceFormItems.paynetServiceFields[this.serviceFormItems.paynetServiceFields.indexOf(item)].fieldAlias = newFields.fieldAlias;
              this.serviceFormItems.paynetServiceFields[this.serviceFormItems.paynetServiceFields.indexOf(item)].fieldType = newFields.fieldType;
              this.serviceFormItems.paynetServiceFields[this.serviceFormItems.paynetServiceFields.indexOf(item)].nameRu = newFields.nameRu;
              this.serviceFormItems.paynetServiceFields[this.serviceFormItems.paynetServiceFields.indexOf(item)].nameEn = newFields.nameEn;
              this.serviceFormItems.paynetServiceFields[this.serviceFormItems.paynetServiceFields.indexOf(item)].nameUz = newFields.nameUz;
              this.serviceFormItems.paynetServiceFields[this.serviceFormItems.paynetServiceFields.indexOf(item)].fieldId = newFields.fieldId;
          ...
          putData = {
            ...
            "nameUz": this.nameUz,
            "nameRu": this.nameRu,
            "serviceId": this.serviceFormItems.serviceId,
            "fieldId": 0,
          }
          this.$http.post(this.$store.getters.newApiUrl + `/payment-service/${this.serviceFormItems.serviceId}/field`, putData)
            .then((response) => {
              this.fieldAlias = '';
              this.fieldType = '';
              this.nameEn = '';
              ...
              this.serviceFormItems.paynetServiceFields = newFields.paynetServiceFields
    ...
    saveEditedService() {
      ...
      if (this.serviceFormItems.locationLongitude)
        this.serviceFormItems.locationLongitude = parseFloat(this.serviceFormItems.locationLongitude.trim().replace(',', '.'))
      if (this.serviceFormItems.locationLatitude)
        this.serviceFormItems.locationLatitude = parseFloat(this.serviceFormItems.locationLatitude.trim().replace(',', '.'))
      ...
      if (self.serviceFormItems.serviceId) {
        ...
        self.$http.put(self.$store.getters.newApiUrl2 + `/payment-service/${self.serviceFormItems.serviceId}`, self.serviceFormItems)
          .then((response) => {
  ...
  mounted() {
    ...
    this.invoiceData = this.serviceFormItems
  ...
  watch: {
    invoiceData(val) {
      Object.assign(this.serviceFormItems, val);
    },
    newImage(image) {
      let self = this;
      if (image.length > 0) {
