            ...
            <div class="other-info">
              <div class="user-info">
                <div class="card-details">
                  ...
                  <div class="other-info-child" v-if="card.details.client">
                    ...
                    <p :class="['other-value', {'cash-payment': !card.details.owner}]">{{
                        card.details.owner |
                          checkStatus
                      }} <span v-if="!card.details.owner">(оформляет на другого человека)</span></p>
                  </div>
                  <div class="other-info-child">
              ...
              <div class="stepper-box">
                <div class="stepper" v-for="(item, index) in steppers" :key="index">
                                    <span class="step">
                                        <p
                                          :class="['step-child', {'active-step' : (index+1)===card.details.step}]">{{ item.step }}</p>
                                        <p
        ...
        <div class="stepper-box history-box">
          <div class="history-stepper" v-for="(item, index) in historyData" :key="index">
            ...
            <span class="history-status">
                            <p
                              :class="['history-status-text', {'active-step-text' : item.createAt}]">{{ item.proposalStatus }}</p>
                            <p
                              :class="['history-status-desc', {'active-step-text' : (index+1)===false}]">{{ item.statusDesc }}</p>
                            <div class="history-time">{{ item.createAt | timestamp-to-date }}</div>

                        </span>
            <span class="history-description">
                            <div class="history-time"
                                 v-if="item.username">{{ item.username }} (ID: {{ item.userId }})</div>
                            <span class="history-time italic">{{ item.details.content }}</span>
                            <span v-if="item.proposalStatus==='REJECTED'"
                                  class="history-time italic">{{ item.details.steps[0].descriptions[0] }}</span>
                        </span>
          </div>
        </div>
        <v-card-actions>
