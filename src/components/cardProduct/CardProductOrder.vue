    ...
    <div class="main">
                ...
                <v-card class="card" v-for="(card, index) in cardOrderData" :key="index">
                    <div class="card-header">
                        ...
                        <span class="change-box">
                            <v-btn v-if="card.status.key==='NEW'" @click="openForwardBox(card)" color="green" style="color: #FFFF; border-radius: 25px;" small depressed>На исполнение</v-btn>
                            ...
                            <v-btn v-if="card.status.key==='WAITING'&&$store.getters.roles!='FRONT_OFFICE'" @click="openBranchBox(card)" color="orange accent-3" style="color: #FFFF; border-radius: 25px;" small depressed>Сменить филиал</v-btn>
                            <v-btn v-if="card.status.key==='WAITING'" @click="openReadyBox(card.id)" color="#1b65be" style="color: #FFFF; border-radius: 25px;" small depressed>Карта выпущена</v-btn>
                            <v-btn v-if="card.status.key==='READY'" @click="openSuccessBox(card.id)" color="green" style="color: #FFFF; border-radius: 25px;" small depressed>Карта передана</v-btn>
                            <v-btn v-if="card.status.key==='NEW'||card.status.key==='UNDER_WRITER' || card.status.key==='WAITING'" @click="openRejectBox(card.id)" color="error" small depressed style="border-radius: 25px">Отказать</v-btn>
                            ...
                            <div class="user-info">
                                <div class="card-details">
                                    ...
                                    <div class="other-info-child">
                                        <p class="other-text">Клиент: <span v-if="card.user.customerId">( ID - {{card.user.customerId}} )</span></p> 
                                    ...
                                    <div class="other-info-child" v-if="card.details.client">
                                        ...
                                        <p :class="['other-value', {'cash-payment': !card.details.owner}]">{{card.details.owner | checkStatus}} <span v-if="!card.details.owner">(оформляет на другого человека)</span></p>
                                    </div>
                                    <div class="other-info-child">
                                    ...
                                    <div :class="['other-info-child']">
                                        <p :class="['other-text', {'delivery': card.details.delivery, 'border-left': card.details.delivery}]">Доставка: </p>
                                        <p :class="['other-value', {'delivery': card.details.delivery, 'border-right': card.details.delivery}]">{{card.details.delivery | checkStatus}}</p>
                                    </div>
                                    <div class="other-info-child">
                                        <p class="other-text">Карта продукта: </p>
                                        <p v-if="card.details.cardProduct" :class="['other-value', {'cash-payment': card.details.cardProduct.id===121||card.details.cardProduct.id===141||card.details.cardProduct.id===142||card.details.cardProduct.id===143||card.details.cardProduct.id===144||card.details.cardProduct.id===145||card.details.cardProduct.id===208||card.details.cardProduct.id===206}]">{{card.details.cardProduct.name}}</p>
                                    </div> 
                                    <div class="other-info-child">
                                    ...
                                    <div class="other-info-child">
                                        ...
                                        <p v-if="card.details.addressId" class="other-value">{{card.details.addressId.city}} <span v-if="card.details.addressId.city">,</span> {{card.details.addressId.address1}} <span v-if="card.details.addressId.address2">, </span> {{card.details.addressId.address2}}</p>
                                    </div>
                                    <div class="other-info-child" v-if="card.details.status">
                                    ...
                                    <div class="other-info-child">
                                        ...
                                        <p :class="['other-value', {'operation-success': card.operationStatus==='SUCCESS', 'operation-failed': card.operationStatus==='FAILED'}]">{{card.operationStatus}}</p>
                                    </div> 
                                ...
                                <div class="doc-box">
                                    <div  class="passport" v-if="card.details.dockFrontUrl||card.details.dockBackUrl||card.details.dockSelfieUrl">              
                                        <!-- <img class="image" v-for="(image, i) in images" :src="image" :key="i" @click="index = i"> -->
                                        <p class="passport-text">Паспортные данные:</p>
                                        <span class="passport-box">
                                            <img v-if="card.details.dockFrontUrl" class="image" :src="card.details.dockFrontUrl"  @click="setImages(card.details, 0)">
                                            <img v-if="card.details.dockBackUrl" class="image" :src="card.details.dockBackUrl"   @click="setImages(card.details, 1)">
                                            <img v-if="card.details.dockSelfieUrl" class="image" :src="card.details.dockSelfieUrl"   @click="setImages(card.details, 2)">
                                        </span>                            
                                    </div>
                                    <div class="passport">
                            ...
                            <div class="stepper-box">
                                <div class="stepper" v-for="(item, index) in steppers" :key="index">
                                    <span class="step" >
                                        ...
                                        <p :class="['step-child-text', {'active-step-text' : (index+1)===card.details.step}]">{{item.name}}</p>
                                    </span>
            ...
            <v-card>
                <v-card-title class="headline">{{dialogText}}</v-card-title>
                        ...
                        <v-card-text style="height: 300px; overflow: auto">                        
                            <v-checkbox class="reasons-box-child" v-model="underWriterId" v-for="(item, index) in reasonsData" :key="index" :label="item.name" :value="item.id"></v-checkbox>                   
                        </v-card-text>
                <v-card-actions>
                    <v-spacer></v-spacer>
                    <v-btn v-if="!isReject" color="blue-grey" style="color: #FFFF;" :loading="loading" text @click="reSubmit">Подтвердить</v-btn>
                    <v-btn v-else color="blue-grey" :loading="loading" style="color: #FFFF;" text @click="reJect">Подтвердить</v-btn>
        ...
        <v-dialog v-model="branchDialog" persistent max-width="560">
            <v-card>
                ...
                <v-card-actions>
                    <v-spacer></v-spacer>
                    <v-btn color="blue-grey"  :loading="loading" style="color: #FFFF;" text @click="changeBranch">Подтвердить</v-btn>
        ...
        <v-dialog v-model="readyDialog" persistent max-width="560">
            <v-card>
                ...
                <v-text-field counter="4" mask="##/##" class="branch-select" max-length="4" label="Срок карты" v-model="expiry" required></v-text-field>  
        ...
        <v-dialog v-model="docDialog" persistent max-width="600">
            <v-card>
                ...
                <div class="pa-3 d-flex" style="flex-wrap: wrap">
                    <span v-for="(item, index) in documents" :key="index" class="ma-1 d-flex" style="flex-direction: column; flex-wrap: wrap; align-items: center">
...
export default {
    ...
    methods:{
        ...
        setComment(val){
            ...
            if(this.comment){
                if(this.isAddComment){
                    self.$http.post(self.$store.getters.newApiUrl + "/card-product-order/info", putData)
                    .then((response) => {
                    ...
                    self.$http.put(self.$store.getters.newApiUrl + "/card-product-order/info", putData)
                    .then((response) => {
                        //console.log(response.data.data);
                        self.isEditComment=false;
                        if(self.isSearch)
        ...
        getSummery(){
            ...
            self.$http.post(self.$store.getters.newApiUrl + "/card-product-order/summery", data)
			.then((response) => {
                //console.log(response.data.data);
        ...
        orderStatus(){
            let self = this;
            self.$http.get(self.$store.getters.newApiUrl + "/card-product-order/status")
			.then((response) => {
                //console.log(response.data.data);
        ...
        getUsers(){
            let self = this;
            self.$http.get(self.$store.getters.newApiUrl + "/card-product-order/user")
			.then((response) => {
                //console.log(response.data.data);
        ...
        getCardOrder(page){
            ...
            self.$http.post(self.$store.getters.newApiUrl + "/card-product-order?page="+(page-1)+'&size=5', postData)
			.then((response) => {
                //console.log(response.data.data);
        ...
        getReasonsList(){
            let self = this;
            self.$http.get(self.$store.getters.apiUrl + "/proposal/card-order/under-writer")
			.then((response) => {
                //console.log(response.data.data);       
        ...
        reSubmit(){
            ...
            self.$http.post(self.$store.getters.apiUrl + `/proposal/card-order/${this.proposalId}/under-writer/resubmit`, postData)
			.then((response) => {
                //console.log(response.data.data);       
                if(self.isSearch)
                    self.orderSearch(self.xPage);
        ...
        reJect(){
            ...
            self.$http.post(self.$store.getters.apiUrl + `/proposal/card-order/${this.proposalId}/under-writer/reject`, postData)
			.then((response) => {
                //console.log(response.data.data);       
                if(self.isSearch)
                    self.orderSearch(self.xPage);
        ...
        forward(){
            let self = this;
            this.loading = true;
            self.$http.post(self.$store.getters.apiUrl + `/proposal/card-order/${this.proposalId}/under-writer/forward?branchId=`+this.branchId)
			.then((response) => {
                //console.log(response.data.data);       
                if(self.isSearch)
                    self.orderSearch(self.xPage);
        ...
        orderSearch(page){
            ...
            self.$http.post(self.$store.getters.newApiUrl + `/card-product-order?page=${page-1}&size=5`, data)
			.then((response) => {
                //console.log(response.data.data);
                this.loader = false;  
                self.cardOrderData = response.data.data.content;
