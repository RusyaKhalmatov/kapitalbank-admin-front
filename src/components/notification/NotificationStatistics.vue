            ...
            >
                ...
                <template slot="items" slot-scope="props">
                    <tr @click="props.expanded = !props.expanded">
                        <td @click="searchUsers(1, props.item.title, props.item.shortDescription, 'TITLEDESC')">{{props.item.id}}</td>
                        <td @click="searchUsers(1, props.item.title, props.item.shortDescription, 'TITLEDESC')">{{props.item.title}}</td>
                        <td @click="searchUsers(1, props.item.title, props.item.shortDescription, 'TITLEDESC')">{{props.item.shortDescription}}</td>
                        <td @click="searchUsers(1, props.item.title, props.item.shortDescription, 'TITLEDESC')">{{props.item.notificationType}}</td>
                        <td @click="searchUsers(1, props.item.title, props.item.shortDescription, 'TITLEDESC')" v-if="props.item.imageUrl"><img class="notifications-image" :src="props.item.imageUrl" alt="image"></td>
                        <td @click="searchUsers(1, props.item.title, props.item.shortDescription, 'TITLEDESC')" v-else></td>
                        <td @click="searchUsers(1, props.item.title, props.item.shortDescription, 'TITLEDESC')">{{props.item.sentDate | timestamp-to-date}}</td>                        
                    </tr>
                </template>
    ...
    export default {
        ...
        methods:{
            ...
            getNotifications(page){
                ...
                let self = this;
                console.log(putData)
                self.$http.put(self.$store.getters.apiUrl + '/notification/search/notifications?page='+(page-1)+'&size=10&search_type='+this.searchType, putData)
                .then(response => {
                    console.log(response.data.data);
