    ...
    <v-flex xs12>
        <v-card min-height="600">
            <v-card-title>
                <h1>Уведомление</h1>
                <v-spacer></v-spacer>
                <v-btn icon small @click="redirect('notificationForm')">
                        ...
                        <v-data-table
                            ...
                            <template slot="items" slot-scope="props">
                                <tr @click="props.expanded = !props.expanded">
                                    ...
                                    <td>
                                        <v-btn icon small
                                               :to="{name: 'notificationForm',params: {notification: props.item}}">
                                            <v-icon small>mdi-pencil</v-icon>
                                        </v-btn>
                                        <v-btn icon small @click="deleteNotification(props.item.id)">
...
<script>
    export default {
        ...
        methods: {
            getAllNotifications() {
                let self = this;
                self.$http.get(self.$store.getters.apiUrl + '/notification/push/all')
                    .then(response => {
                        self.loader = false;
            ...
            deleteNotification(id) {
                let self = this;
                self.$http.delete(self.$store.getters.apiUrl + `/notification/${id}`)
                    .then(() => {
